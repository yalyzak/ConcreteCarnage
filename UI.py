...
class UI:
    def attach(self, owner_object):
        """Initialize and attach all UI components to parent object."""
...
class AbstractUI:
    ...
    def __init__(self):
        """Initialize UI with empty button list."""
        self.show = False
        self.buttons = []

    def Start(self):
        """Initialize render system and setup UI elements."""
    ...
    def setup_layout(self):
        """Set up the UI layout (can be overridden by subclasses)."""
        self.usePing()

    def usePing(self):
        """Add ping display text to UI."""
        self.render.add_text_rect(AbstractUI.ping_text)

    @staticmethod
    def updatePing(ping):
        """Update ping text display with new value."""
        AbstractUI.ping_text.text = f"{ping} MS"

    def CloseLayout(self):
        """Close and clear all UI elements."""
        self.Active = False
        self.show = False
        self.render.flush_ui()

    def Update(self, dt):
        """Handle UI updates and button clicks."""
        if not self.show:
            self.setup_layout()
